...
private:
  ...
  void ir_sensor_callback(const sensor_msgs::msg::Range &msg)
  {
    ...
    if (msg.range < 0.1) {
      ...
      cc_target_speed_ind = (1+cc_target_speed_ind)%3;
  ...
  void command_loop_function(void)
  {
    if(turn_time > 0) {
      ...
      RCLCPP_INFO(this->get_logger(), "CC Target Speed: %0.3f", cc_target_speeds[cc_target_speed_ind]);
  ...
  float cc_target_speeds[] = {0.1, 0.2, 0.4};
  int cc_target_speed_ind = 0;
